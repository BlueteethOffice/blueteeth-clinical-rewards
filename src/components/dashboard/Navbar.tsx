            ...
            <motion.div initial={{ opacity: 0, y: 10, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.98 }} className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden z-50 p-1">
              {isSearching ? (
                <div className="p-8 flex flex-col items-center justify-center gap-2 text-slate-400">
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-wider">Searching...</p>
                </div>
              ) : (searchResults.cases.length > 0 || searchResults.users.length > 0 || searchResults.pages.length > 0) ? (
                <div className="max-h-[28rem] overflow-y-auto custom-scrollbar">
                  {searchResults.pages.length > 0 && (
                    <div className="p-1">
                      <p className="px-3 py-2 text-[9px] font-bold text-slate-400 uppercase tracking-wider">Navigation</p>
                      {searchResults.pages.map(p => (
                        ...
                        >
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Open Page</p>
                          </div>
                        </button>
                      ))}
                    </div>
                  ...
                  {searchResults.cases.length > 0 && (
                    <div className="p-1">
                      <p className="px-3 py-2 text-[9px] font-bold text-slate-400 uppercase tracking-wider">Clinical Cases</p>
                      {searchResults.cases.map(c => (
                        <button key={c.id} onClick={() => handleCaseSelect(c)} className="w-full text-left p-3 hover:bg-slate-50 dark:hover:bg-white/5 rounded-lg transition-all group flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <span className="text-[9px] font-bold uppercase text-slate-400 block">{c.status}</span>
                  ...
                  {searchResults.users.length > 0 && (
                    <div className="p-1 border-t border-slate-50 dark:border-white/5 mt-1">
                      <p className="px-3 py-2 text-[9px] font-bold text-slate-400 uppercase tracking-wider">Platform Users</p>
                      {searchResults.users.map(u => (
                        ...
                        >
                          ...
                          <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 rounded-full text-[8px] font-bold text-slate-400 uppercase tracking-wider">{u.role}</span>
                        </button>
                      ))}
                    </div>
                  )}
                ...
                <div className="p-10 text-center text-slate-400 font-bold text-[10px] uppercase tracking-wider">No matching records</div>
              )}
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div className="flex items-center gap-2 sm:gap-4 lg:gap-8">
        ...
        <div className="relative" ref={notificationRef}>
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 sm:-top-1 sm:-right-1 min-w-4 h-4 px-1 bg-red-600 text-white text-[9px] font-bold rounded-full border-2 border-white dark:border-slate-900 flex items-center justify-center shadow-sm leading-none">
            ...
            {showNotifications && (
              ...
              >
                <div className="p-4 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50">
                  <h3 className="text-[10px] font-bold text-slate-900 dark:text-white uppercase tracking-wider">Notifications</h3>
                  {unreadCount > 0 && <button onClick={markAllAsRead} className="text-[9px] font-bold text-cyan-600 uppercase tracking-wider">Clear All</button>}
                </div>
                ...
                <Link href="/dashboard/notifications" onClick={() => setShowNotifications(false)}>
                  <div className="p-3 bg-slate-50 dark:bg-white/5 text-center text-[9px] font-bold text-slate-400 uppercase tracking-wider border-t">View All activity</div>
                </Link>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="relative" ref={profileMenuRef}>
          <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="flex items-center gap-2 sm:gap-4 pl-1 sm:pl-6 sm:border-l sm:border-slate-100 sm:dark:border-slate-800 group">
            <div className="text-right hidden md:block">
              <p className="text-xs font-bold text-slate-900 dark:text-slate-100 group-hover:text-cyan-600 transition-colors">{formatName(user?.displayName || user?.name || 'User', user?.role || '')}</p>
            ...
            {showProfileMenu && (
              <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className="absolute right-0 mt-3 w-56 bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-100 dark:border-slate-800 p-2 z-50">
                <div className="px-3 py-2 bg-slate-50 dark:bg-slate-800 rounded-lg mb-2">
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Session ID</p>
      ...
      {showLogoutModal && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-10 w-full max-w-sm z-10 border border-slate-100 dark:border-white/5">
            <h2 className="text-base font-bold text-slate-900 dark:text-white text-center mb-8 uppercase tracking-[0.2em]">Confirm Exit?</h2>
            <div className="flex gap-3">
              <button onClick={() => setShowLogoutModal(false)} disabled={isLoggingOut} className="flex-1 py-3.5 text-[10px] font-bold uppercase tracking-wider bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl hover:bg-slate-200 transition-all">No</button>
              <button onClick={handleLogout} disabled={isLoggingOut} className="flex-1 py-3.5 text-[10px] font-bold uppercase tracking-wider bg-red-500 text-white rounded-xl shadow-lg shadow-red-500/20 hover:bg-red-600 transition-all flex items-center justify-center gap-2">
