  ...
  return (
    <DashboardLayout>
      <div className="max-w-6xl mx-auto">
        <div className="mb-10">
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tighter uppercase">Earnings Vault</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
          <div className="lg:col-span-1 bg-slate-900 rounded-xl p-8 text-white shadow-2xl relative overflow-hidden group">
            ...
            <p className="text-slate-500 font-black text-[10px] uppercase tracking-widest">Available Earnings</p>
            <h2 className="text-5xl font-black mb-6 tracking-tighter">₹{earnings}</h2>
            
            <div className="flex items-center gap-2 mb-8 text-[9px] font-black uppercase tracking-widest bg-white/5 w-fit px-3 py-2 rounded-lg border border-white/5">
            ...
            <button 
              ...
              className="w-full py-4 bg-cyan-600 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-xl shadow-cyan-900/20 hover:bg-cyan-500 active:scale-95 transition-all disabled:opacity-50"
            >
          ...
          <div className="lg:col-span-2 glass-card rounded-xl p-8 border border-slate-100 dark:border-white/5 bg-white dark:bg-slate-900/40">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="px-3 py-1 bg-slate-50 dark:bg-slate-800 rounded-lg text-[9px] font-black text-slate-400 uppercase">
            ...
            <div className="overflow-x-auto custom-scrollbar -mx-6 sm:mx-0 px-6 sm:px-0">
              {loading ? (
                <div className="py-20 flex flex-col items-center gap-4">
                  ...
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Syncing History...</p>
                </div>
              ) : payouts.length > 0 ? (
                <table className="w-full text-left whitespace-nowrap min-w-[500px]">
                  <thead>
                    <tr className="border-b border-slate-50 dark:border-white/5">
                      <th className="pb-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Amount</th>
                  ...
                  <tbody className="divide-y divide-slate-50 dark:divide-white/5">
                    {payouts.map((p) => (
                      <tr key={p.id} className="group transition-colors">
                        <td className="py-5 font-black text-slate-900 dark:text-white">₹{p.amount}</td>
                        <td className="py-5 text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-tighter">
                          {p.createdAt?.toDate ? format(p.createdAt.toDate(), 'dd MMM, yyyy') : 'Recently'}
                        </td>
                        <td className="py-5 text-right">
                          <span className={`px-2.5 py-1 rounded text-[9px] font-black uppercase tracking-widest inline-flex items-center gap-1.5 ${
