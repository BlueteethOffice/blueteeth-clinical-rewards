    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
          ...
          <p className="text-slate-400 font-bold text-xs uppercase tracking-widest">Building your workspace...</p>
        </div>
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6 mb-8 sm:mb-10">
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tighter uppercase">Associate Hub</h1>
            <div className="text-slate-500 dark:text-slate-400 font-bold mt-1 uppercase text-[9px] sm:text-[10px] tracking-widest flex items-center gap-2">
              <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
              Live performance monitoring for {user?.name}
            </div>
          </div>
          <Link href="/dashboard/associate/submit-case">
            <button className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-6 sm:px-8 py-3 sm:py-4 rounded-lg sm:rounded-xl font-black text-[10px] sm:text-xs uppercase tracking-[0.16em] shadow-xl shadow-slate-200 dark:shadow-none flex items-center justify-center gap-2 sm:gap-3 hover:scale-[1.02] active:scale-95 transition-all w-full sm:w-auto min-w-40">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 md:gap-6 mb-10">
          <StatCard title="Submissions" value={stats.total.toString()} icon={ClipboardList} color="bg-blue-600" />
          <StatCard title="Reviewing" value={stats.pending.toString()} icon={Clock} color="bg-amber-500" />
          <StatCard title="Active" value={stats.active.toString()} icon={Activity} color="bg-indigo-600" />
          <StatCard title="Approved" value={stats.approved.toString()} icon={CheckCircle2} color="bg-emerald-500" />
        </div>

        <div className="mb-6 flex items-center gap-4">
          <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest whitespace-nowrap">Finance Engine</p>
          <div className="h-px bg-slate-100 dark:bg-white/5 w-full" />
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 md:gap-6 mb-12">
          <StatCard title="Earnings" value={`₹${(stats.grossEarnings || 0).toLocaleString()}`} icon={TrendingUp} color="bg-cyan-600" />
          <StatCard title="Payout" value={`₹${(stats.withdrawableAmount || 0).toLocaleString()}`} icon={Wallet} color="bg-emerald-600" />
          <StatCard title="Paid Out" value={`₹${(stats.totalWithdrawn || 0).toLocaleString()}`} icon={ArrowDownCircle} color="bg-slate-900" />
          <StatCard title="Points" value={`${stats.pendingPoints || 0} PTS`} icon={Coins} color="bg-amber-600" />
        </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-2">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <h3 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight whitespace-nowrap">Recent Cases</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap">Real-time update</p>
                </div>
              </div>
              <Link href="/dashboard/associate/my-cases" className="group flex items-center gap-2 text-cyan-600 font-black text-[10px] uppercase tracking-widest hover:text-cyan-700 transition-all sm:ml-auto">
            ...
            <div className="space-y-4">
              {paginatedCases.length > 0 ? (
                          ...
                          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <div className="flex items-center gap-2">
                                  <span className="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight group-hover:text-cyan-600 transition-colors">{c.patientName}</span>
                                  <span className="text-[9px] font-black text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">#{c.id.slice(-6).toUpperCase()}</span>
                                </div>
                            ...
                            <div className="flex items-center gap-6">
                              <div className="text-right hidden sm:block">
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Reward Value</p>
                                <p className="text-sm font-black text-slate-900 dark:text-white mt-0.5">₹{(c.points || 0) * 50}</p>
                              </div>
                              <div className="text-right min-w-[100px]">
                                <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest mb-1 inline-block ${
                  ...
                  {hasMore && (
                    <div className="flex items-center justify-center pt-6">
                      <button 
                        ...
                        disabled={loading}
                        className="px-8 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-[1.02] active:scale-95 transition-all shadow-lg flex items-center gap-2"
                      >
                ...
                <div className="glass-card p-16 rounded-2xl border-dashed border-2 border-slate-200 dark:border-slate-800 text-center flex flex-col items-center gap-4 bg-slate-50/50 dark:bg-white/5">
                  ...
                  <h4 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest">Log is Empty</h4>
          ...
          <div className="space-y-6">
            <div className="glass-card rounded-2xl p-6 sm:p-8 bg-slate-900 text-white relative overflow-hidden shadow-2xl min-h-[300px] sm:min-h-[320px] flex flex-col justify-between border-none">
              ...
              <div>
                ...
                <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em]">Withdrawable Balance</p>
                <h2 className="text-4xl font-black tracking-tighter mt-1">₹{(stats.withdrawableAmount || 0).toLocaleString()}</h2>
              </div>
              <Link href="/dashboard/associate/earnings" className="block w-full">
                <button className="w-full py-4 bg-cyan-600 text-white rounded-xl font-black text-xs uppercase tracking-[0.2em] hover:bg-cyan-700 transition-all flex items-center justify-center gap-2">
            ...
            <div className="glass-card p-6 min-h-44 rounded-2xl border border-slate-100 dark:border-white/5 flex items-center justify-end gap-4 text-right">
              ...
              <div className="max-w-60">
                <h4 className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest">Earning Tip</h4>
                ...
                <div className="mt-4 inline-flex items-center justify-center rounded-lg bg-indigo-50 dark:bg-indigo-500/10 px-3 py-2 text-[9px] font-black uppercase tracking-widest text-indigo-600 dark:text-indigo-300">
