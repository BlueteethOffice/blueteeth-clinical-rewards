    ...
    return (
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
          ...
          <p className="text-slate-400 font-black text-[10px] uppercase tracking-widest">Loading Wallet...</p>
        </div>
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout>
      <div className="max-w-6xl mx-auto pb-10">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 sm:mb-10 px-4 lg:px-0">
          <div>
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tighter uppercase">Earnings Log</h1>
          ...
          <div className="inline-flex flex-col sm:items-end px-5 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-white/5 rounded-xl self-start sm:self-auto">
            ...
            <p className="text-sm sm:text-base font-black text-slate-900 dark:text-white tracking-tight">₹{pointValue} <span className="text-[10px] text-slate-400">/ PT</span></p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-10">
          <div className="glass-card p-6 sm:p-8 rounded-xl border border-slate-100 dark:border-white/5 relative overflow-hidden group bg-white dark:bg-slate-900/40">
            ...
            <div className="relative z-10">
              ...
              <p className="text-slate-400 dark:text-slate-500 text-[9px] sm:text-[10px] font-black uppercase tracking-widest">Total Approved Points</p>
              <h3 className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-white mt-1 tracking-tighter">{stats.approvedPoints} <span className="text-[10px] sm:text-xs text-slate-400 tracking-widest uppercase">Points</span></h3>
              <div className="mt-3 sm:mt-4 flex items-center gap-3">
                <span className="flex items-center gap-1 text-[8px] sm:text-[9px] font-black text-amber-500 bg-amber-50 dark:bg-amber-500/10 px-2 sm:px-2.5 py-1 rounded-lg uppercase tracking-widest"><Clock size={10} /> {stats.pendingPoints} Pending</span>
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-2 relative overflow-hidden rounded-xl p-6 sm:p-8 shadow-xl shadow-slate-200 dark:shadow-none group bg-slate-900 border-none">
            ...
            <div className="relative z-10 h-full flex flex-col sm:flex-row sm:items-center justify-between gap-6 sm:gap-8">
              <div>
                <p className="text-cyan-400 font-black text-[9px] sm:text-[10px] uppercase tracking-[0.3em] mb-3 sm:mb-4">Available for Payout</p>
                <h2 className="text-4xl sm:text-5xl md:text-6xl font-black text-white tracking-tighter flex items-center gap-2">
                  <IndianRupee size={32} className="text-cyan-500 sm:w-10 sm:h-10" strokeWidth={3} />
                  {stats.withdrawableAmount.toLocaleString()}
                </h2>
                <div className="mt-4 sm:mt-6 flex items-center gap-3 sm:gap-4 text-[8px] sm:text-[9px] font-black text-slate-500 uppercase tracking-widest">
              ...
              <button 
                ...
                className="w-full sm:w-auto px-6 sm:px-8 py-4 sm:py-5 bg-cyan-600 text-white rounded-xl font-black text-[10px] sm:text-xs uppercase tracking-[0.2em] shadow-xl shadow-cyan-900/40 hover:bg-cyan-500 transition-all disabled:opacity-50 active:scale-95 whitespace-nowrap"
              >
                Withdraw Funds
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="glass-card rounded-xl p-6 sm:p-8 border border-slate-100 dark:border-white/5 bg-white dark:bg-slate-900/40 overflow-hidden">
              ...
              {payouts.length > 0 ? (
                    ...
                    <table className="w-full text-left whitespace-nowrap min-w-[500px]">
                      <thead>
                        <tr className="border-b border-slate-50 dark:border-white/5">
                          <th className="px-4 pb-4 text-[9px] font-black text-slate-400 uppercase tracking-widest text-left">ID</th>
                          <th className="px-4 pb-4 text-[9px] font-black text-slate-400 uppercase tracking-widest text-left">Amount</th>
                          <th className="px-4 pb-4 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Status</th>
                        </tr>
                      </thead>
                        ...
                        {payouts.map(p => (
                          <tr key={p.id} className="group">
                            <td className="px-4 py-4">
                              <p className="text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-tight">PAY-{p.id.slice(-6).toUpperCase()}</p>
                              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{p.createdAt?.toDate ? format(p.createdAt.toDate(), 'dd MMM') : 'Just now'}</p>
                            </td>
                            <td className="px-4 py-4">
                              <p className="text-sm font-black text-slate-900 dark:text-white tracking-tighter">₹{p.amount.toLocaleString()}</p>
                    ...
                    {payouts.map(p => (
                      <div key={p.id} className="p-4 rounded-xl border border-slate-100 dark:border-white/5 bg-slate-50/30 dark:bg-white/2 space-y-3">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-tight">PAY-{p.id.slice(-6).toUpperCase()}</p>
                            <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">{p.createdAt?.toDate ? format(p.createdAt.toDate(), 'dd MMM, HH:mm') : 'Just now'}</p>
                          </div>
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-white/5">
                          <p className="text-sm font-black text-slate-900 dark:text-white tracking-tighter">₹{p.amount.toLocaleString()}</p>
                          <p className="text-[8px] font-black text-slate-400 uppercase">{p.paymentMethod}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="py-16 text-center text-slate-400 font-black text-[10px] uppercase tracking-widest">No payout records found.</div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="glass-card p-8 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-white/5">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-6">Ledger Summary</p>
              <div className="space-y-6">
                <div>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Lifetime Earned</p>
                  <p className="text-2xl font-black text-slate-900 dark:text-white mt-1 tracking-tight">₹{stats.grossEarnings.toLocaleString()}</p>
                </div>
                <div className="h-px bg-slate-200 dark:bg-white/5" />
                <div>
                  <p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest">Total Settled</p>
                  <p className="text-2xl font-black text-emerald-600 mt-1 tracking-tight">₹{stats.totalWithdrawn.toLocaleString()}</p>
                </div>
                <div className="h-px bg-slate-200 dark:bg-white/5" />
                <div>
                  <p className="text-[9px] font-black text-amber-500 uppercase tracking-widest">In Pipeline</p>
                  <p className="text-2xl font-black text-amber-500 mt-1 tracking-tight">₹{stats.pendingPayouts.toLocaleString()}</p>
                </div>
              </div>
            </div>
          </div>
              ...
              <form onSubmit={handleWithdraw} className="p-8 space-y-6">
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input type="number" required min={500} max={stats.withdrawableAmount} value={withdrawAmount} onChange={(e) => setWithdrawAmount(e.target.value)} className="w-full pl-8 pr-4 py-4 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-white/5 rounded-xl font-black text-sm outline-none focus:border-cyan-600 text-slate-900 dark:text-white" placeholder="Min 500" />
                  </div>
                </div>

                <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-lg">
                  <button type="button" onClick={() => setPaymentMethod('upi')} className={`flex-1 py-3 rounded text-[10px] font-black uppercase tracking-widest transition-all ${paymentMethod === 'upi' ? 'bg-white dark:bg-slate-700 text-cyan-600 shadow-sm' : 'text-slate-400'}`}>UPI</button>
                  <button type="button" onClick={() => setPaymentMethod('bank')} className={`flex-1 py-3 rounded text-[10px] font-black uppercase tracking-widest transition-all ${paymentMethod === 'bank' ? 'bg-white dark:bg-slate-700 text-indigo-600 shadow-sm' : 'text-slate-400'}`}>Bank Transfer</button>
                </div>

                {paymentMethod === 'upi' ? (
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">UPI ID</label>
                    <input type="text" placeholder="e.g. name@upi" value={upiId} onChange={(e) => setUpiId(e.target.value)} className="w-full px-4 py-4 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-white/5 rounded-xl font-black text-sm outline-none focus:border-cyan-600 text-slate-900 dark:text-white" />
                  </div>
                ) : (
                  <div className="space-y-4">
                    <input type="text" placeholder="Holder Name" value={bankDetails.holderName} onChange={(e) => setBankDetails({...bankDetails, holderName: e.target.value})} className="w-full px-4 py-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-white/5 rounded-xl font-black text-xs outline-none focus:border-indigo-600" />
                    <div className="grid grid-cols-2 gap-4">
                      <input type="text" placeholder="A/C No" value={bankDetails.accountNumber} onChange={(e) => setBankDetails({...bankDetails, accountNumber: e.target.value})} className="w-full px-4 py-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-white/5 rounded-xl font-black text-xs outline-none focus:border-indigo-600" />
                      <input type="text" placeholder="IFSC" value={bankDetails.ifsc} onChange={(e) => setBankDetails({...bankDetails, ifsc: e.target.value})} className="w-full px-4 py-3.5 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-white/5 rounded-xl font-black text-xs outline-none focus:border-indigo-600" />
                    </div>
                  </div>
                )}

                <button type="submit" disabled={requesting} className="w-full py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:scale-[1.01] transition-all flex items-center justify-center gap-2">
